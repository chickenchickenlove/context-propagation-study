...
@Slf4j
public class MitigationAsynchronizedContextPropagation {
    ...
    static void handleRequest() {
        ...
        CompletableFuture<Void> future = CompletableFuture
                .runAsync(new WrappedRunnable(() -> addProduct("test-product")), forkJoinPool) // Main 쓰레드가 생성함 -> Context 넘어감. -> Worker-1번이 실행함
                .thenRunAsync(new WrappedRunnable(() -> notifyShop("test-product")), forkJoinPool); // Worker-1번이 실행함 -> Context 넘어감. -> Worker 2번이 실행함.
